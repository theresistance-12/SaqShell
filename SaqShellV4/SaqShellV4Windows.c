    ...
    while (fish) {
        ...
        if (fgets (shell_input, sizeof(shell_input), stdin)== 0) {
            ...
            if (hFind == INVALID_HANDLE_VALUE) {
                ...
                do {
                    // findData.cFileName ist das Gegenstück zu d_name
                    printf(" %s\n", findData.cFileName);

                } while (FindNextFile(hFind, &findData) != 0); // Suche weiter, bis nichts mehr da ist

                FindClose(hFind); // Den "Handle" (Pointer) wieder schließen
            }
        } else {
